...
```
  ...
  * Modificar un elemento especifico de un vector
...
```
  ...
  * Extrar los elementos de las posiciones en los que los valores **NO** son NA
...
```
  ...
  * Extrar las posiciones del vector que cumplan con una consulta
...
El error que retorna **R** es que el tipo character no es un argumento válido, esto significa que los datos que nosotros ingresamos son del tipo caracter, para comprobar esto, usaremos la función `type()`:

```{r}
typeof(matriz[,2])
```

Se puede observar que efectivamente, los datos que intentamos ingresar **R** los tiene almacenados como tipo caracter, por lo que, para poder sumar los datos utilizaremos la función `as.numeric` (existen otras funciones como `as.integer`, `as.double()` que tambien podrían funcionar):
...
Pero que pasa si no se busca nombrar las filas o las columnas, lo recomendable sería usar las funciones respectivas `colnames(...)` o `rownames(...)`.
...
> Observe que el vector `Puntos` a pesar de que le faltó una coma para separar el valor `20` de `30`, **R** lo sigue tomando como valido por que no rompe con la sintaxis del código, sin embargo, esto nos causará problemas más adelante.
...
```{r dataframe1}
data.frame(columna1 = 1, columna2 = 1:5, columnay = LETTERS[1:5], z=runif(5))
```

#### Crear un Data frame

* Crear un data frame apartir de varios vectores:
...
* Crear un data frame apartir de una matriz:
...
Continuaremos trabajando con el siguiente data frame, es necesario recordar que se puede llamar de cualquier forma, en este caso se llamará de manera generica, `df` :
...
Lo más probable es que el directorio de trabajo de **R** no sea el mismo que donde están los datos, la función `getwd()` sirve para revisar cuál esl el directorio de trabajo actual, será necesario revisar nuevamente los primeros pasos de esté capitulo para corregir el problema con el directorio de trabajo.

###### ¿Donde están los datos?

Cuando se utilizan las funciones `read.table(...)` o `load(...)` utilizan la ruta del directorio de trabajo actual para guardar los datos, es decir, que si esa ruta apunta a tus documentos, los datos se guardaran en esa carpeta, será necesario revisar nuevamente los primeros pasos de esté capitulo para ver como funciona el directorio de trabajo.

###### ¿Cuales son los datos que cargué con .RData?

Si por algun motivo se desconoce cuales son los datos que se están cargando (No vienen documentados de alguna manera, etc.), la función `load(...)` incluye el parametro `verbose` que por default el `FALSE`, pero si cambiamos dicho parametro a `TRUE`, nos dira que el nombre del objeto que estamos cargando al entorno de **R**, así podremos llamarlo y ver que es lo que contiene.
...
Dicha libreria no se encuentra instalada por defecto en **R** para instalarla, como se vio en los primeros pasos de este capitulo, se usará el comando:

```{r, eval=F}
install.packages("dplyr")
```

Para aprender a usar esta libreria usaremos el comando:
```{r, eval=F}
library(dplyr)
```
...
Ahora filtraremos por los animales mamíferos y reptiles que sean carnivoros y tengan un tiempo de vida mayor a 19 años:
...
La función `arrange(...)` funciona de manera similiar a `filter(...)` solo que, en vez de filtrar los datos, los ordena.
...
De manera seguida tendremos que trabajar con grandes cantidades de datos con varias columnas, pero en el momento, solo algunas podran ser de tu interés la función `select(...)` permite un rápido trabajo para seleccionar dichas columnas, por ejemplo, queremos conocer el tiempo de vida de cada animal, entonces:
...
Para renombrar una columna, la libreria `dplyr` provee tambien una útil función llamada `rename(...)`, con ella cambiaremos el nombre de la columna `Nombre` del data frame `Animales` por `Animal`:
...
Otra útil función dentro de está librería es la función `distinct(...)` que encuentra los valores distintos en una tabla, por ejemplo las Especies de los animales:
...
o tambien las especies y alimentación 
...
Es posible modificar las columnas existentes o agregar nuevas columnas con la función `mutate(...)` de la librería `dplyr`, por ejemplo, agregaremos dos años al tiempo de vida de los animales y añadiremos tambien otra columna con datos `NA`.
